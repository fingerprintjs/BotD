...
const runDetection = async () => {
  ...
  try {
    ...
    const components = (instance as BotDetector)["components"];
    ...
    const resultTableEntries = [
      [
        { text: "automationTool" },
        result.bot
          ? { text: "<b>Detected</b>", class: "green" }
          : { text: "<b>Not detected</b>" },
        result.bot
          ? { text: `<b>${result.botKind}</b>`, class: "green" }
          : null,
      ],
    ];
        ...
        (row) =>
          `<tr>${row
            ...
            .map(
              (column) =>
                `<td style="width: 25%" ${
                  column?.class != null ? `class=${column.class}` : ""
                }>${column?.text}</td>`,
            )
            .join("")}</tr>`,
      )
